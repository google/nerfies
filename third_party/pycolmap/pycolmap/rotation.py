...
class Quaternion:
  ...
  def __mul__(self, other):
    if isinstance(other, Quaternion):  # quaternion multiplication
      return Quaternion(
          np.array((self.q[0] * other.q[0] - self.q[1] * other.q[1] -
  ...
  def ToR(self):
    return np.eye(3) + 2 * np.array((
        (-self.q[2] * self.q[2] - self.q[3] * self.q[3], self.q[1] * self.q[2] -
         self.q[3] * self.q[0], self.q[1] * self.q[3] + self.q[2] * self.q[0]),
        (self.q[1] * self.q[2] + self.q[3] * self.q[0], -self.q[1] * self.q[1] -
         self.q[3] * self.q[3], self.q[2] * self.q[3] - self.q[1] * self.q[0]),
  ...
  def ToAxisAngle(self):
    ...
    if np.abs(sin_sq_theta) > np.finfo('float').eps:
      ...
      angle = 2. * (
          np.arctan2(-sin_theta, -cos_theta) if cos_theta < 0. else np.arctan2(
              sin_theta, cos_theta))
...
class DualQuaternion:
  ...
  def inverse(self):
    ...
    return ~self * complex(inv_len_real,
                           -normsq.imag * inv_len_real * inv_len_real)
