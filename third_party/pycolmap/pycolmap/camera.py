...
def opencv_distortion(camera, x):
  ...
  return x * (1. + r_sq * (camera.k1 + camera.k2 * r_sq)) + np.concatenate(
      (2. * camera.p1 * xy + camera.p2 * (r_sq + 2. * x_sq), camera.p1 *
       (r_sq + 2. * y_sq) + 2. * camera.p2 * xy),
      axis=-1)
...
class Camera:
  ...
  def __str__(self):
    s = (
        self.GetNameFromType(self.camera_type) +
  ...
  def get_inverse_camera_matrix(self):
    return np.array(((1. / self.fx, 0, -self.cx / self.fx),
                     (0, 1. / self.fy, -self.cy / self.fy), (0, 0, 1)))
